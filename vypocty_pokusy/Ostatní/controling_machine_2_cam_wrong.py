...
def execute_command_and_measure(port, distance, period, camera, x_lim, y_lim):
    if not port.is_open:
        ...
        with io.open(file_path, 'w', encoding='utf-8') as file:
            ...
            command = f"MMDIC2 {distance} {period}"
...
def make_measurement(camera_index, port, output_folder, x_limit=1, y_limit=1, command_input1=0, command_input2=0,
    ...
    [cv2.imwrite(os.path.join(output_folder, f"Frame_{i + 1: 03d}.jpg"), frame) for i, frame in enumerate(photos)]
...
def main():
    ...
    while True:
        ...
        folder_path = os.path.join(output, name, "detail_original")
