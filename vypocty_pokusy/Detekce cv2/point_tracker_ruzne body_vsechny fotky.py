...
if data_type == 1:
    ...
    x = range(2650, 3450, 80)  # Doplnění x-ových souřadnic bodů
    y = range(400, 650, 25)  # Doplnění y-ových souřadnic bodů
...
if data_type == 5 or 6:
    ...
    if data_type == 6:
        ...
        x_min, y_min = 400, 650   # Levý horní roh oblasti
        x_max, y_max = 2650,  3450    # Pravý dolní roh oblasti

        # Vytvoření masky pro omezení oblasti
        Mask = np.zeros_like(gray1)
        Mask[y_min:y_max, x_min:x_max] = 0  # maskování
...
cv2.resizeWindow(window_name_1, width, height)
cv2.resizeWindow(window_name_2, width, height)
