...
if source_type == 'webcam':
    ...
    properties = [camera.get(prop_id) for prop_id in range(19)]  # range(cv2.CAP_PROP_POS_MSEC, cv2.CAP_PROP_MODE + 1):
...
if decoded_object:
    ...
    for s in range(n):
        points = mark_area_on_canvas(cv2.cvtColor(reference_image, cv2.COLOR_BGR2RGB), selecting_function="polygon",
                                     face_color="dodgerblue", edge_color="navy", name=f"Mark QR codes [{s + 1} / {n}]")
