...
use_logo = False
make_frame = True
add_short_name = False

square_size = 10

# Vytvoření QR kódu s určenou úrovní korekce chyb
error_correction = qrcode.constants.ERROR_CORRECT_Q  # Oprava chyb: L (Low), M (Medium), Q (Quartile), H (High)
...
for i in range(1, 3):
    ...
    data = f".*SP*._N#{i}"
    # f"F04_{i:03d}"   f"T02_{i:02d}-I_1s"   "H02_{i:02d}_10s"   ".*CP*._N#3" 02_{i:02d}-I-max_12s_k
    name = f"support{i}"  # "Measurement_" + data   "calibration_point05"  "qr_Friction_test_" "qr_Hex_test_"
    short_name = data
    ...
    qr = qrcode.QRCode(
        ...
        error_correction=qrcode.constants.ERROR_CORRECT_H if use_logo else error_correction,  # Úroveň korekce chyb
        # Oprava chyb: L (Low), M (Medium), Q (Quartile), H (High)
