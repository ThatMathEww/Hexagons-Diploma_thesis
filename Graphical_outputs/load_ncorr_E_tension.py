...
for i in range(len(indexes)):
    ...
    axs[i].set_xlabel(r"Strain [%]")
    axs[i].set_ylabel(r"Stress [$MPa$]")
...
plt.gca().set_ylabel(r"Stress [$MPa$]")
plt.grid()
# plt.legend()
plt.tight_layout()
...
for n, (name, curve_index, color) in enumerate(datas_pack):
    ...
    ax1.fill_between(data_mean_x, data_mean_y + data_std, data_mean_y - data_std, alpha=0.35, color=color, zorder=10 + n)
    ...
    for i in range(len(data_plot_x)):
        ...
        module = (data_plot_y[i][index_max_strain] - data_plot_y[i][1]) / (data_plot_x[i][index_max_strain] - data_plot_x[i][1])
...
for axes in [ax1, ax2]:
    ...
    axes.set_xlabel(r"Strain [%]")
    axes.set_ylabel(r"Stress [$MPa$]")
